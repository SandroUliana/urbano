	...
	init:{
		...
		Home:function(){ // init
			...
			if($.isEmptyObject(APP.TABDATA)){
				...
				$("#POPmappaTxt").html('<strong>La tabella dei metadati risulta essere vuota.</strong><br>Non &egrave possibile continuare, provare a ricaricare l&#39;applicazione')						
				$("#POPmappa").popup("open")
				}
			else{
				APP.MAP.HOME.POS=JSON.parse(JSON.stringify(APP.MAP.DEFAULT.POSITION))
				if(APP.MAP.HOME.GETPOSITION){
					navigator.geolocation.getCurrentPosition(
						function(position){
							APP.MAP.HOME.POS.LAT=position.coords.latitude;
							APP.MAP.HOME.POS.LNG=position.coords.longitude;					
							APP.TAB=false
							APP.MAP.HOME.draw()
							},
						function(){
							APP.MAP.HOME.draw()
							$("#POPmappaTxt").html('<strong>Il rilevamento di posizione risulta essere non attivo o non disponibile.</strong><br>Attivare la geolocalizzazione sul dispositivo o spostarsi in una zona con migliore ricezione del segnale GPS')						
							$("#POPmappa").popup("open")		
							},
						{timeout:10000})
					APP.MAP.HOME.GETPOSITION=false			
					}
				else{
					APP.MAP.HOME.draw()
					}
		...
		Posizione:function(){ // init	
			...
			if(APP.ASSET.Id){
				...
				// ricerca su mappa
				$("body").append($("<input>",{"type":"text","id":"inputSearchP","style":"margin-top:10px;height:30px;"}))
				APP.MAP.SEARCHBOXP=new google.maps.places.SearchBox(document.getElementById('inputSearchP'))
				APP.MAP.MAPPOS.controls[google.maps.ControlPosition.TOP_LEFT].push(document.getElementById('inputSearchP'))
				APP.MAP.SEARCHBOXP.addListener('places_changed',function(){
					navigator.geolocation.clearWatch(APP.MAP.WATCH);
					var places = APP.MAP.SEARCHBOXP.getPlaces();	
					if(places.length==0){return;}else{
						...
						APP.MAP.MAPPOS.fitBounds(bounds);
						APP.MAP.MAPPOS.reload()
						}
					})				
				// start watchposition
	...
	MAP:{		
		HOME:{
			...
			OPTIONS:{
				...
				zoom: 10,
				styles:[
					{stylers: [{ hue: "#2d5d84" },{ saturation: -20 }]},
			...
			MARKERS:[],
			draw:function(){
				if(!APP.TAB){APP.UTIL.ajax("GET","urbano/api/urbobjs/","my_lat="+APP.MAP.HOME.POS.LAT+"&my_long="+APP.MAP.HOME.POS.LNG+"&zoom="+APP.MAP.HOME.ZOOM+((APP.MAP.HOME.LAYER.CAT)?"&category="+APP.MAP.HOME.LAYER.CAT:"")+((APP.MAP.HOME.LAYER.TIPO)?"&tipology="+APP.MAP.LAYER.TIPO:""),function(RESULT){APP.TAB=APP.PARSE.tab(RESULT)},true)}
							
				if(!APP.MAP.HOME.MAP){
					APP.MAP.MARKERS=[]	
					var coords=
					APP.MAP.HOME.MAP = new google.maps.Map(
						document.getElementById('mappaGoogle'),
						{center: new google.maps.LatLng(APP.MAP.HOME.POS.LAT,APP.MAP.HOME.POS.LNG)},
						APP.MAP.HOME.OPTIONS
						);				
					$("#mappaGoogle").show()
					// searchbox
					$("body").append($("<input>",{"type":"text","id":"inputSearchHome","style":"margin-top:10px;height:30px;"}))
					APP.MAP.HOME.SEARCHBOX=new google.maps.places.SearchBox(document.getElementById('inputSearchHome'))
					APP.MAP.HOME.MAP.controls[google.maps.ControlPosition.TOP_LEFT].push(document.getElementById('inputSearchHome'))
					APP.MAP.HOME.SEARCHBOX.addListener('places_changed',function(){				
						var places = APP.MAP.HOME.SEARCHBOX.getPlaces();	
						if(places.length==0){return;}else{
							var bounds = new google.maps.LatLngBounds();
							if (places[0].geometry.viewport) {bounds.union(places[0].geometry.viewport);} 
							else{bounds.extend(places[0].geometry.location);}				
							APP.MAP.HOME.MAP.fitBounds(bounds);
							APP.MAP.HOME.reload()
							}
						})
					// SPIDERFIER
					var oms = new OverlappingMarkerSpiderfier(APP.MAP.HOME.MAP);
					var iw = new google.maps.InfoWindow();
					oms.addListener('click', function(marker, event) {
						iw.setContent(marker.desc);
						iw.open(APP.MAP.HOME.MAP, marker);
						});
					//
					if(APP.TAB.length>0){
						$.each(APP.TAB,function(IND,ASSET){
							var MARKER= new google.maps.Marker({
								position:{lat:ASSET.Coordinate.Lat,lng:ASSET.Coordinate.Lng},
								map: APP.MAP.HOME.MAP,							
								icon: APP.UTIL.getIcon(ASSET.Categoria,ASSET.Tipologia)	
								})
							var dataora=ASSET.Data.split(" ")
							MARKER.desc='<a href="javascript:APP.MAP.scheda(&quot;'+ASSET.Id+'&quot;)" class="BTNmark">'+
										...
										'</a>';
							APP.MAP.HOME.MARKERS.push(MARKER)
							oms.addMarker(MARKER);							
							})
						if(APP.MAP.MARKERS.length>0){
							var markerCluster = new MarkerClusterer(APP.MAP.HOME.MAP,APP.MAP.HOME.MARKERS,{
								maxZoom:18,
								zoomOnClick:true,
								styles:[{
									url: 'IMG/cluster.png',
							...
							APP.MAP.zoom(APP.MAP.HOME.MAP,APP.MAP.HOME.MARKERS)
							}
						window.setTimeout(function(){	
							APP.MAP.HOME.MAP.addListener('zoom_changed',function(){
								if(APP.MAP.HOME.MAPINFOW){APP.MAP.HOME.MAPINFOW.close()};
								APP.MAP.HOME.ZOOMTYPE="ALL"
								});
							},500);
						}
					else{ // xxx
						...
						str+=(APP.MAP.HOME.LAYER.CAT||APP.MAP.HOME.LAYER.TIPO)?
